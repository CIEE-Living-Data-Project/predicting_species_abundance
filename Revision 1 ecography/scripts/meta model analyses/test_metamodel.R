...
# we can re-run models on subset of data that doesn't have big differences between indivs or sp
...
# (subset data below like 10,000 indivs diff and to less than 175 sp diffs, breaks based on diff x corr plots above)

# model 1
start_time <- Sys.time() 
metamod.indivs <- brm(MODFORM.indivs, moddat,
             control = list(adapt_delta=0.8, max_treedepth = 12), cores=3, chains=3, 
             iter=5000, family=gaussian, prior = priors, 
             file="Revision 1 ecography/output/meta model/SEindivs.rmd")
end_time <- Sys.time()
end_time - start_time
