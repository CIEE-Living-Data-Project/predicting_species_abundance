...
# model 2
start_time <- Sys.time() 
metamod.sp2 <- brm(MODFORM.sp, moddat, 
                  cores=3, chains=3, 
                  threads = threading(6), # sets number of threads per chain, this will use 3*6 = 18 threads out of 32 total
                  prior = priors, 
                  #control = list(adapt_delta=0.9, max_treedepth = 11), 
                  iter=6000, warmup = 4000, family=gaussian,  backend = "cmdstanr",
                  file="Revision 1 ecography/output/meta model/SEspecies_2K_threads.rmd")
end_time <- Sys.time()
end_time - start_time
